...
# This renders a single right eye (centered on screen) and
...
if config.full_screen:
    DISPLAY = pi3d.Display.create(samples=4)  # Full screen
    DISPLAY.set_background(0, 0, 0, 0)  # r,g,b,alpha
else:
    DISPLAY = pi3d.Display.create(samples=4, w=config.display_width, h=config.display_height)
...
async def main():
    ...
    sub = await nc.subscribe("eyes")

    while True:
        try:
            async for msg in sub.messages:
                shared = json.loads(msg.data)
                frame()
                if sub.pending_msgs > 5:
                    print("Backlog too long. Reconnecting...")
